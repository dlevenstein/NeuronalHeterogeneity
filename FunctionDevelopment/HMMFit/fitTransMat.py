...
def main():
    ...
    for state in range(len(brainstates)):
        ...
        if index.size:
            ...
            out[brainstates1[state]] = {'UID':UID+1,'basepath':basepath_original, 'decoded_mode':decoded_mode, 'state_isi':state_isi, 'state_label':state_names, 'state_spk':state_spk, 'logrates':logrates, 'cvs':cvs, 'trans_mat':model.dense_transition_matrix()[:logrates.size+1,:logrates.size]}
