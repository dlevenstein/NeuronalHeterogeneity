...
for cc = 1:spikes.numcells
    MI_rate(cc) = mutualinfo(spkmat.data(:,cc),spkmat.pos);
end
...
[ISIbyPOS_norm(cc)] = bz_ConditionalISI(spikes.times{cc},position_norm,...
    ...
    'normtype','none','Xwin',[-0.75 0.75],'minX',10);
end

MIthresh = 0.013;
ISIbyPOS_norm_mean = bz_CollapseStruct( ISIbyPOS_norm(squeeze(ISIbyPOS.MutInf)>MIthresh & MI_rate'>MIthresh),3,'mean',true);
...
MIforbest(MI_rate<MIthresh) = nan;
[~,bestcell] =max(MIforbest);
[~,sortMI_ISI] = sort(ISIbyPOS.MutInf);
[~,sortMI_rate] = sort(MI_rate);
%%
figure
subplot(2,2,1)
...
subplot(2,2,2)
    imagesc(ISIbyPOS_norm_mean.Dist.Xbins,[1 spikes.numcells],squeeze(log10(ISIbyPOS_norm.Dist.SpikeRate(:,:,sortMI_rate)))')
...
subplot(2,2,4)
    plot(log10(MI_rate),squeeze(log10(ISIbyPOS.MutInf)),'.')
    xlabel('MI - rate');ylabel('MI - ISI')
    NiceSave('PlaceCoding',figfolder,baseName,'includeDate',true)
