...
for ss = 1:2
for cc = 1:numcells
        ModeInts.(states{ss}).cells(cc) = bz_IDXtoINT(ModeHMM.(states{ss})(cc).prev_state',...
        ...
        ModeInts_time.(states{ss}).cells(cc).(modenames{sm}) = ...
            [ModeHMM.(states{ss})(cc).state_spk(nexttemp(:,1)) ModeHMM.(states{ss})(cc).state_spk(ModeInts.(states{ss}).cells(cc).([modenames{sm},'state'])(:,2)]);
        end
end
end
...
[ exwin ] = bz_RandomWindowInIntervals( SleepState.ints.WAKEstate,20,1 )

figure
subplot(6,1,1)
StateScorePlot( ModeInts_time.WAKEstate.cells(cc),modecolors )
hold on
for sm = 1:6
    %if ss==6
        instate_both = ModeHMM.WAKEstate(cc).prev_state == sm & ModeHMM.WAKEstate(cc).next_state==sm;
    %else
        instate_either = ModeHMM.WAKEstate(cc).prev_state == sm | ModeHMM.WAKEstate(cc).next_state==sm;
    %end
    plot(ModeHMM.WAKEstate(cc).state_spk(instate_either),ones(size(ModeHMM.WAKEstate(cc).state_spk(instate_either))),...
        '+','color',[0.5 0.5 0.5])
    plot(ModeHMM.WAKEstate(cc).state_spk(instate_both),ones(size(ModeHMM.WAKEstate(cc).state_spk(instate_both))),'k+')
