function [ ] = Analysis20190228(basePath,figfolder)
% Date XX/XX/20XX
%
...
for ss = 1:2
        ...
        imagesc(log2(SpikeLFPCoupling(8).(states{ss}).freqs),...
            ...
            squeeze(SpikeLFPCoupling(8).(states{ss}).(inout{io}).spikephasemag)');
       ...
       % end 
        ...
        imagesc(log2(SpikeLFPCoupling(8).(states{ss}).freqs),...
            [1 length(spikeGroups.groups{8})],...
            squeeze(SpikeLFPCoupling(8).(states{ss}).(inout{io}).ratepowercorr)');
        ...
        imagesc(log2(SpikeLFPCoupling(8).(states{ss}).freqs),...
            [1 length(spikeGroups.groups{8})],...
            squeeze(SpikeLFPCoupling(8).(states{ss}).(inout{io}).ISIpowermodulation)');
