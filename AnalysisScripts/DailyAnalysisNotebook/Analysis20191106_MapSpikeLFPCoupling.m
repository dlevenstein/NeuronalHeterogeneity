...
basePath = '/home/dlevenstein/ProjectRepos/NeuronalHeterogeneity/Datasets/onProbox/YS_CTX/YMV02_170815';
%basePath = pwd;
...
allclasses = unique(AllCellClass(~cellfun(@isempty,AllCellClass)));
classcolors = {'k','r','k','r'};
classline = {'-','-','--','--'};
%%
...
for ss = 1:3
    subplot(3,3,1+(ss-1)*3)
        imagesc(log10(SpikeLFPCoupling.(states{ss}).freqs),[0 1],SpikeLFPCoupling.(states{ss}).pop.vCTX_pI.phasemag')
        LogScale('x',10)
        colorbar
        
    subplot(3,3,2+(ss-1)*3)
        imagesc(log10(SpikeLFPCoupling.(states{ss}).freqs),[0 1],SpikeLFPCoupling.(states{ss}).pop.vCTX_pI.powercorr')
        LogScale('x',10)
        colorbar
        
    subplot(3,3,3+(ss-1)*3)
        imagesc(log10(SpikeLFPCoupling.(states{ss}).freqs),[0 1],SpikeLFPCoupling.(states{ss}).pop.vCTX_pI.ISIpowermodulation')
        LogScale('x',10)
        colorbar
    ...
    for ss = 1:numstates
       ...
       for tt = 1:length(allclasses)
           SpikeLFPCoupling(gg).(state).pop.(allclasses{tt}).ISIpowermodulation = ...
               squeeze(mean(SpikeLFPCoupling(gg).(state).cell.ISIpowermodulation(...
               strcmp(AllCellClass,allclasses{tt}),:,:),1));
       end
    end
end
