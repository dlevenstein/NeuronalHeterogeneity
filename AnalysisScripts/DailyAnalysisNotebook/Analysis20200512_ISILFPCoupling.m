...
for ss = 1:3
    subplot(3,3,1+(ss-1)*3)
        imagesc(log10(SpikeLFPCoupling.(states{ss}).freqs),[0 1],SpikeLFPCoupling.(states{ss}).pop.CTX_pI.phasemag')
        LogScale('x',10)
        colorbar
        
    subplot(3,3,2+(ss-1)*3)
        imagesc(log10(SpikeLFPCoupling.(states{ss}).freqs),[0 1],SpikeLFPCoupling.(states{ss}).pop.CTX_pI.powercorr')
        LogScale('x',10)
        colorbar
        
    subplot(3,3,3+(ss-1)*3)
        imagesc(log10(SpikeLFPCoupling.(states{ss}).freqs),[0 1],SpikeLFPCoupling.(states{ss}).pop.CTX_pI.ISIpowermodulation')
        LogScale('x',10)
        colorbar
