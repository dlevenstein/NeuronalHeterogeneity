function [] = FanoFactorAnalysis(basePath,figfolder)
...
%%
binsizes = logspace(-2,1.5,10); %s
overlap = 1;
for bb = 1:length(binsizes)
    bb
    spikemat = bz_SpktToSpkmat(spikes,'binsize',binsizes(bb),'overlap',overlap);

    %%
    ss = 1;
    spikemat.instate = InIntervals(spikemat.timestamps,SleepState.ints.(statenames{ss}));

    %%
    FFstats.mean = mean(spikemat.data(spikemat.instate,:),1);
    FFstats.var = var(spikemat.data(spikemat.instate,:),0,1);
    FFstats.multiFF(bb,:) = FFstats.var./FFstats.mean;

end
%%
figure
plot(log10(binsizes),log10(FFstats.multiFF),'o-')
LogScale('xy',10)
xlabel('Bin Size (s)');ylabel('FF')
...
%%
FFstats.mean = mean(spikemat.data(spikemat.instate,:),1);
FFstats.std = std(spikemat.data(spikemat.instate,:),[],1);
FFstats.FF = (FFstats.std.^2)./FFstats.mean;
...
subplot(2,2,2)
plot(log10(ISIStats.summstats.NREMstate.meanCV2),FFstats.FF,'.')
