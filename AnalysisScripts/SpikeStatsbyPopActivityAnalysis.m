...
for nn = 1:4
for st = 1:length(synchtypes)
PopRatebyPSS.(normtypes{nn}).(synchtypes{ss})  = ConditionalHist(...
    spikemat.BSmetrics.PSS,spikemat.poprate.(normtypes{nn}).(synchtypes{ss}),...
    'Xbounds',[0 1],'numXbins',25,'numYbins',100,'minX',2000,...
    'Ybounds',popratehist.(normtypes{nn}).bins.(synchtypes{st})([1 end]));

PopRatebyTheta.notNREM.(normtypes{nn}).(synchtypes{ss})  = ConditionalHist(...
    spikemat.BSmetrics.thratio(~spikemat.instate.NREMstate),...
    spikemat.poprate.(normtypes{nn}).(synchtypes{ss})(~spikemat.instate.NREMstate),...
    'Xbounds',[0 1],'numXbins',25,'numYbins',100,'minX',2000,...
    'Ybounds',popratehist.(normtypes{nn}).bins.(synchtypes{st})([1 end]));

PopRatebyTheta.WAKE.(normtypes{nn}).(synchtypes{ss})  = ConditionalHist(...
    spikemat.BSmetrics.thratio(spikemat.instate.WAKEstate),...
    spikemat.poprate.(normtypes{nn}).(synchtypes{ss})(spikemat.instate.WAKEstate),...
    'Xbounds',[0 1],'numXbins',25,'numYbins',100,'minX',2000,...
    'Ybounds',popratehist.(normtypes{nn}).bins.(synchtypes{st})([1 end]));

PopRatebyTheta.REM.(normtypes{nn}).(synchtypes{ss})  = ConditionalHist(...
    spikemat.BSmetrics.thratio(spikemat.instate.REMstate),...
    spikemat.poprate.(normtypes{nn}).(synchtypes{ss})(spikemat.instate.REMstate),...
    'Xbounds',[0 1],'numXbins',25,'numYbins',100,'minX',2000,...
    'Ybounds',popratehist.(normtypes{nn}).bins.(synchtypes{st})([1 end]));

%[-1.5 1.75]
% [ PopRatebyTheta.(synchtypes{ss}) ] = cellfun(@(X,Y,Z,W) ConditionalHist( [Z(~W);Z(~W)],log10([X(~W);Y(~W)]),...
...
figure
subplot(2,2,1)
imagesc(PopRatebyPSS.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    PopRatebyPSS.(normtypes{nn}).(synchtypes{ss}).Ybins,...
    PopRatebyPSS.(normtypes{nn}).(synchtypes{ss}).pYX')
hold on
plot(PopRatebyPSS.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    bz_NormToRange(PopRatebyPSS.(normtypes{nn}).(synchtypes{ss}).pX),'k')
axis xy
xlabel('PSS');ylabel('Pop Rate (norm)')

subplot(2,2,2)
imagesc(PopRatebyTheta.notNREM.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    PopRatebyTheta.notNREM.(normtypes{nn}).(synchtypes{ss}).Ybins,...
    PopRatebyTheta.notNREM.(normtypes{nn}).(synchtypes{ss}).pYX')
hold on
plot(PopRatebyTheta.notNREM.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    bz_NormToRange(PopRatebyTheta.notNREM.(normtypes{nn}).(synchtypes{ss}).pX),'k')
axis xy
xlabel('Theta');ylabel('Pop Rate (norm)')


subplot(4,2,6)
imagesc(PopRatebyTheta.WAKE.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    PopRatebyTheta.WAKE.(normtypes{nn}).(synchtypes{ss}).Ybins,...
    PopRatebyTheta.WAKE.(normtypes{nn}).(synchtypes{ss}).pYX')
hold on
plot(PopRatebyTheta.WAKE.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    bz_NormToRange(PopRatebyTheta.WAKE.(normtypes{nn}).(synchtypes{ss}).pX),'k')
axis xy
ylabel({'WAKE','Pop Rate'})

subplot(4,2,8)
imagesc(PopRatebyTheta.REM.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    PopRatebyTheta.REM.(normtypes{nn}).(synchtypes{ss}).Ybins,...
    PopRatebyTheta.REM.(normtypes{nn}).(synchtypes{ss}).pYX')
hold on
plot(PopRatebyTheta.REM.(normtypes{nn}).(synchtypes{ss}).Xbins,...
    bz_NormToRange(PopRatebyTheta.REM.(normtypes{nn}).(synchtypes{ss}).pX),'k')
axis xy
