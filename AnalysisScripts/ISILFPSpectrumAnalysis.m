function [PSSConditionalISIDist,MutInf,HiLowISIStats,...
    PSSConditionalGamma,GammaParms,PSScorr,PSShist] = ISILFPSpectrumAnalysis(basePath,figfolder)
% Date XX/XX/20XX
%
%Question: 
%
    ...
    for ff = 1:length(specslope.freqs)
        ...
        fPower(ff).data = downsample(specslope.resid(:,ff),3);
            ...
            [cdist] = bz_ConditionalISI(spikes.times,fPower(ff),...
                'ints',SleepState.ints.(states{ss}),...
                'showfig',false,'GammaFit',false,'minX',0,'ISIDist',false,'numISIbins',100);
...
%% Compare Smooth Windows for PSS...
% smoothwins = 1; %s
...
for ss = 1:3
    ...
    PSShist.(states{ss}) = hist(smoothdata(instate),PSShist.bins);
    totaltime = totaltime + sum(PSShist.(states{ss}));
    ...
    tempstruct = bz_ISIStats(spikes,'ints',INT,'showfig',false);
     ...
     HiLowISIStats.numspks.(states{ss}) = cellfun(@(spk) length(spk),tempstruct.allspikes.times);
...
%%
for ss = 1:3
    PSShist.(states{ss}) = PSShist.(states{ss})./totaltime;
end
%%
figure
hold on
for ss = 1:3
plot(PSShist.bins,PSShist.(states{ss}),'color',statecolors{ss})
end
NiceSave('PSShist',figfolder,baseName)
%%
