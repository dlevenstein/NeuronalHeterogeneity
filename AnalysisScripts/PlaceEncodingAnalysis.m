...
binsize = 0.25;
spkmat = bz_SpktToSpkmat(spikes.times,'dt',0.05,'binsize',binsize,'win',position.Epochs.MazeEpoch,'units','counts');
...
for cc = 1:spikes.numcells
    ...
    CONDXY(cc) = ConditionalHist(spkmat.pos,spkmat.data(:,cc),...
         'numXbins',100,'Xbounds',[0 3],'numYbins',maxspikes+1,'Ybounds',[0 maxspikes],...
         'Xbinoverlap',3);
end
...
for cc = 1:length(HDcells)
    subplot(4,3,cc)
        ...
        xlabel('Position');ylabel('Spike Count')
        title(['UID: ',num2str(spikes.UID(HDcells(cc)))])

%         if mod(cc,12)==0
%             NiceSave(['PlaceCells_SpikeCount',num2str(counter)],figfolder,baseName)
%             counter = counter +1;
%             figure
%         end
end
NiceSave(['PlaceCells_SpikeCount_dt',num2str(binsize)],figfolder,baseName)
